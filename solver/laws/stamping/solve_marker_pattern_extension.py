...
def solve_one(inp):
    ...
    for c in template_reg['colors']:
        if any(any(inp[r, col] == c for r, col in other['coords']) for other in other_regions):
            marker_color = c
            break
    if marker_color is None: return inp
...
def solve_marker_pattern_extension(solver):
    """
    Law interface for MegaSolver.
    """
    ...
    # Return predictions for test inputs
    return [solve_one(inp) for inp in solver.test_in]

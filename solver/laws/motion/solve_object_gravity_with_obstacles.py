...
def solve_object_gravity_with_obstacles(solver) -> Optional[List[np.ndarray]]:
    for dr, dc in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
        for bg in range(10):
            for obstacle_color in range(10):
                if obstacle_color == bg: continue
                consistent = True; found_any_move = False
                for inp, out in solver.pairs:
                    res = inp.copy(); moved = True
                    while moved:
                        moved = False; coords = np.argwhere((res != bg) & (res != obstacle_color))
                        ...
                        for r, c in coords:
                            ...
                            if 0 <= nr < res.shape[0] and 0 <= nc < res.shape[1] and res[nr, nc] == bg:
                                res[nr, nc] = res[r, c]; res[r, c] = bg; moved = True; found_any_move = True
